...
class RobotService:
    ...
    def init_service(self, variables: Variables) -> None:
        ...
        """
        self.timezone_offset = variables.timezone_offset
        if self.rp is None:
    ...
    def start_launch(
        ...
        sl_pt = {
            ...
            "rerun": rerun,
            "rerun_of": rerun_of,
            "start_time": to_timestamp(ts or launch.start_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
        }
    ...
    def finish_launch(self, launch: Launch, ts: Optional[str] = None) -> None:
        ...
        """
        fl_rq = {
            "end_time": to_timestamp(ts or launch.end_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
    ...
    def start_suite(self, suite: Suite, ts: Optional[str] = None) -> Optional[str]:
        ...
        start_rq = {
            ...
            "start_time": to_timestamp(ts or suite.start_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
        }
    ...
    def finish_suite(self, suite: Suite, issue: Optional[str] = None, ts: Optional[str] = None) -> None:
        ...
        """
        fta_rq = {
            "end_time": to_timestamp(ts or suite.end_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
            "issue": issue,
    ...
    def start_test(self, test: Test, ts: Optional[str] = None):
        ...
        start_rq = {
            ...
            "start_time": to_timestamp(ts or test.start_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
    ...
    def finish_test(self, test: Test, issue: Optional[str] = None, ts: Optional[str] = None):
        ...
        fta_rq = {
            "attributes": test.attributes,
            "end_time": to_timestamp(ts or test.end_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
            "issue": issue,
    ...
    def start_keyword(self, keyword: Keyword, ts: Optional[str] = None):
        ...
        start_rq = {
            ...
            "start_time": to_timestamp(ts or keyword.start_time, self.timezone_offset)
    ...
    def finish_keyword(self, keyword: Keyword, issue: Optional[str] = None, ts: Optional[str] = None):
        ...
        """
        fta_rq = {
            "end_time": to_timestamp(ts or keyword.end_time, self.timezone_offset) or datetime.now(tz=timezone.utc),
            "issue": issue,
    ...
    def log(self, message: LogMessage, ts: Optional[str] = None):
        ...
        sl_rq = {
            ...
            "time": to_timestamp(ts or message.timestamp, self.timezone_offset) or datetime.now(tz=timezone.utc),
        }
        try:
            self.rp.log(**sl_rq)
        except Exception as e:
